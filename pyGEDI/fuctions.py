...
def gediDownload(
    outdir: str, product: str, version: str, bbox: str, session: str
):
    make_dir(outdir)
    url = 'https://lpdaacsvc.cr.usgs.gov/services/gedifinder?product=' + \
        product+'&version='+str(version)+'&bbox='+str(bbox)+'&output=json'
...
def idsBox(fileh5: str, latlayer: str, lonlayer: str, bbox: str):
    ids = []
...
def waveForm(shot_number, fileh5):
    ...
    rx_sample_start_index_n = rx_sample_start_index - \
        min(rx_sample_start_index)+1

    rx_sample_start = int(rx_sample_start_index_n[shot_number_id])
    rx_sample_end = int(
        rx_sample_start_index_n[shot_number_id] + rx_sample_count[shot_number_id]-1)
    ...
    step = (elevation_bin0_i-elevation_lastbin_i) / \
        rx_sample_count[shot_number_id]
